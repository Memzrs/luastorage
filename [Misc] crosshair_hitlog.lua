...
local function on_paint()
	...
	for i = 1, #shots do
		local r, g, b = 0, 0, 0
		a = ((globals_curtime() - shots[i][3]) - time) <= 0 and 254 or 0
		--local debug = string.format("alpha: %d | condition: %s | curtime: %s | shottime: %s | uiTime: %s", a, ((globals_curtime() - shots[i][3]) - time), globals_curtime(), shots[i][3], time)
		--print(debug)
