...
local unsafe_key = ui_new_hotkey("LUA", "A", "Unsafe Mode")
local tpp_key = ui_new_hotkey("LUA", "A", "Ideal Tick")
...
for i=1, #config_names do
    rage[i] = {
        ...
        prefer_baim_disablers = ui_new_multiselect("LUA", "A", "[" .. config_names[i] .. "] Prefer body aim disablers", {"Low inaccuracy", "Target shot fired", "Target resolved", "Safe point headshot"}),
        ...
        tpp_enable = ui_new_checkbox("LUA", "A", "[" .. config_names[i] .. "] Teleport peek"),
        tpp_md = ui_new_slider("LUA", "A", "[" .. config_names[i] .. "] Teleport peek min dmg", 0, 126, 20, true, nil, 1, damage_idx),
    }
end
...
rage[scoped_wpn_idx[1]].js_prefer_baim_disablers = ui_new_multiselect("LUA", "A", "[Jump Scout] Prefer body aim disablers", {"Low inaccuracy", "Target shot fired", "Target resolved", "Safe point headshot"})
...
--#endregion
local ref_baimpl = ui.reference("PLAYERS", "Adjustments", "Override prefer body aim")
local ref_pl = ui.reference("PLAYERS", "Players", "Player list")
local timer = 20
...
local function run_adjustments()
    ...
    local idealtick_on = (ui_get(rage[active_idx].tpp_enable) and ui_get(tpp_key)) and true or false
    ...
    for i=1, #players do
        ...
        local hp = entity.get_prop(entindex, "m_iHealth")
        local vel = { entity.get_prop(entindex, "m_vecVelocity") }
        local sqrt_vel = math.sqrt(vel[1]^2 + vel[2]^2)
        local flags = entity.get_prop(entindex, "m_fFlags")

        

        if (hp < 91 and idealtick_on) or (sqrt_vel < 80 and sqrt_vel > 5 and idealtick_on and flags ~= 256 and flags ~= 263) then
            ui.set(ref_pl, entindex)
            ui.set(ref_baimpl, "On")
            draw_flag = true
        else
            ui.set(ref_baimpl, "-")
            draw_flag = false
...
client.register_esp_flag("IDEAL", 255, 0, 0, function(e)
    local idealtick_on = (ui_get(rage[active_idx].tpp_enable) and ui_get(tpp_key)) and true or false

    local hp = entity.get_prop(e, "m_iHealth")
    local vel = { entity.get_prop(e, "m_vecVelocity") }
    local sqrt_vel = math.sqrt(vel[1]^2 + vel[2]^2)
    local flags = entity.get_prop(e, "m_fFlags")
...
local function run_visuals()
    ...
    local leth_rev = (ui_get(lethal_rev) and active_idx == 3 and #enemies > 0) or false
    --renderer_text(cx - 3, cy - 6, 255, 255, 255, 255, "b", 0, "*")
    if ui_get(rage[active_idx].tpp_enable) and ui_get(tpp_key) then
        local perc = math.floor(c_cmd / 14 * 100 + 0.5)

        if perc == 0 then
            perc = 7
        end
        
        renderer_text(cx, cy - 50, 255, 255, 255, 255, "cd-", 0, "IDEAL TICK")
        renderer_text(cx, cy - 40, 255, 255, 255, 255, "cd-", 0, "CHARGED APPROX (" .. perc.. "%)")
        -- renderer_text(cx + 9, cy + 35, 255, 255, 255, 255, "dc-", 0, "CHARGED IDEAL TICK " .. tostring(ui_get(ref_mindamage)))
    end
    ...
    if ui_get(unsafe_key) then
        renderer_text(cx, cy - 30, 255, 0, 0, 255, "cd-", 0, "DANGEROUS")
    end
end
...
local function handle_menu()
    ...
    ui_set_visible(tpp_key, enabled)
    ui_set_visible(unsafe_key, enabled)
    ...
    for i=1, #config_names do
        ...
        ui_set_visible(rage[i].tpp_enable, show)
        ui_set_visible(rage[i].tpp_md, show and ui_get(rage[i].tpp_enable))
    end
...
local function set_config(idx)
    ...
    if ui_get(rage[i].tpp_enable) and ui_get(tpp_key) then
        damage_val = ui_get(rage[i].tpp_md)
    ...
    if ui_get(unsafe_key) then
        ui_set(ref_prefer_safepoint, false)
        ui_set(ref_multipoint_scale, 100)
        ui_set(ref_multipoint, {"Head", "Chest", "Stomach", "Arms", "Legs", "Feet"})
		ui.set(ref_unsafe, {})
    else
...
local function init_callbacks()
    ...
    for i=1, #config_names do
        ...
        ui_set_callback(rage[i].tpp_enable, handle_menu)
    end
