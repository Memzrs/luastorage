...
local function handlegui()
	local on = ui.get(slot6)

	if on then
		ui.set_visible(slot7, true)
	else
...
function slot11()
	...
	if slot2 <= entity.get_prop(slot0, "m_iAccount") then
		client.exec(slot1)
		client.color_log(0, 255, 0, "[negr0buy] Attempted to autobuy weapons. ", globals.curtime())
	else
		client.color_log(255, 0, 0, "[negr0buy] Insufficient funds for autobuy.")
	end
end
